...
int main() {
    ...
    cvtColor(frame, imgGrayscale, CV_BGR2GRAY);        // convert to grayscale
    ...
    for (int i = 0; i < ptContours.size(); i++) {                           // for each contour
        if (contourArea(ptContours[i]) > MIN_CONTOUR_AREA) {                // if contour is big enough to consider
            ...
            cv::rectangle(frame, boundingRect, cv::Scalar(0, 0, 255), 2);      // draw red rectangle around each contour as we ask user for input
            ...
            imshow("frame", frame);       // show training numbers image, this will now have red rectangles drawn on it
    ...
    FileStorage fsClassifications("/Users/xianlingzhang/Downloads/opencv-3.0.0/Code/OpenCV_VideoCapture/OpenCV_VideoCapture/classifications.xml", cv::FileStorage::WRITE);           // open the classifications file
    ...
    FileStorage fsTrainingImages("/Users/xianlingzhang/Downloads/opencv-3.0.0/Code/OpenCV_VideoCapture/OpenCV_VideoCapture/images.xml", FileStorage::WRITE);         // open the training images file
